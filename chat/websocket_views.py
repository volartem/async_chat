...
from .models import get_room_by_id, create_message, get_user_by_username
        ...
        if not token:
            ...
            while True:
                ...
                if msg.type == aiohttp.WSMsgType.text:
                    if user:
                        user_db = await get_user_by_username(conn, user)
                        message = await create_message(conn, msg.data, user_db['id'], room_id)
                    else:
                    ...
                    for ws in request.app['websockets'][room_id].values():
                        if user:
                            await ws.send_json(
                                {'action': 'sent', 'username': user, 'text': message['message'], 'created': message['created']})
                else:
                    break
